    ...
    setSearch(e) {
      if (this.inputSearch.length >= 3) {
        axios.get('/api/search-product', {
          ...
          if (res.data.success) {
            this.setSearchProducts(res.data.data);
          } else {
        ...
        <div class="main-header__container">
          ...
          <div class="main-header__col col-2">
            ...
            <div class="main-header-search">
              <ClientOnly>
                <input type="text" @change="setSearch" @keydown.enter="setSearch" v-model="inputSearch"
                  :placeholder="isDesktop ? 'Поиск народных товаров' : 'Поиск'">
              </ClientOnly>
              <div class="main-header-search__icon main-search__js default-anim" @click="setSearch" v-anim-hover>
              ...
              <div class="main-header-user-nav-item">
                <template v-if="$route.fullPath === '/order'">
                  <router-link to="/basket/" class="main-header-user-nav-item__body">
                    ...
                    <div class="main-header-user-nav-item__count bg-green hidden header-total-price__js">
                    </div>
                  </router-link>
                </template>
                <template v-else>
                  <div class="main-header-user-nav-item__body open-popup-basket__js">
                    ...
                    <div class="main-header-user-nav-item__count bg-green" :class="{ 'hidden': (cartQuantity < 1) }">
                      {{ cartQuantity }}
                    </div>
                  </div>
                </template>
