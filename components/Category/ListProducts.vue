...
export default {
    async setup(props, context) {
        ...
        const filters = computed(() => {
            ...
            for (const key of ['price_max', 'price_min', 'in_stock']) {
        ...
        if (f.price_max) query['filters[0][price_max]'] = f.price_max
        ...
        context.emit('getCount', products.value.length);


        const productsPerPage = ref(8);
        const displayProducts = ref(products.value.slice(0, productsPerPage.value))
        const hasMore = ref(products.value.length > productsPerPage.value)
    ...
    <section class="list-products block-products">
        <div class="list-products__container container">
            <div class="list-products__items list-products__js g-4">
                <template v-for="(product) in displayProducts" :key="'categoryProducts-'+product.id">
