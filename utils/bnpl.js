...
export function initBnplModal() {
    ///////////////////////////
    ////// Модальное окно
    ///////////////////////////
    const bnplModal = document.querySelector('.bnpl-modal');
    if (!bnplModal) return; // проверка на существование элемента
...
export function resetBnpl() {
    // Виджеты с расписанием
    const bnplScheduleCards = document.querySelectorAll('.bnpl-card--schedule')

    bnplScheduleCards.length && bnplScheduleCards.forEach((card) => {
        card.querySelector('.bnpl-schedule-calc .bnpl-schedule-calc__part').remove()
    })
}
